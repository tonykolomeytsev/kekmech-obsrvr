...
R_flip = -np.eye((3,3),dtype=np.float32)
R_flip[0,0]=1.0
...
while True:
	...
	if ids != None and ids[0] == id_to_kind:
		
		# ret = [rvec, tvec, ?]
		# array of rotation and position of each marker in camera frame
		# rvec = [[rvec_1], [rvec_2], ...] attitude of the marker respect
		# tvec = [[tvec_1], [tvec_2], ...] position of the marker
		ret = aruco.estimatePoseSingleMarkers(corners, marker_size, camera_matrix, camera_distortion)

		# unpack the output, get only the first
		rvec, tvec = ret[0][0,0,:], ret[1][0,0,:]

		#draw the detected ,arker and put a reference frame over it
